...
function SelectAllOpenOrder({
	...
	const allOpenOrdersIds = deliveriesForSelectedCommunity
		.filter(_delivery => {
			const {
				order: { status }
			} = _delivery
	...
	function showSelectAllContainer() {
		return (
			allOpenOrdersIds.length > 0 &&
			isEqualArray(statusFilterList, openOrdersArray)
		)
	}
...
function SelectAllOuForDelivery({
	...
	const allOutForDeliveryOrdersIds = deliveriesForSelectedCommunity
		.filter(_delivery => {
			const {
				order: { status }
			} = _delivery
	...
	function showSelectAllBtn() {
		return !(
			selectedOutforDeliveryOrders.length === allOutForDeliveryOrdersIds.length
		)
	}

	function showSelectAllContainer() {
		return (
			allOutForDeliveryOrdersIds.length > 0 &&
			isEqualArray(statusFilterList, outForDeliveryArray)
		)
	}
